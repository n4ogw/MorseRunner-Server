...
type
  ...
  published
    ...
    property AttackSamples: integer
      read FAttackSamples write SetAttackSamples default 28;
...
begin
  ...
  FIsOverload := FIsOverload or (Abs(Result.Re) = FMaxOut) or
    (Abs(Result.Im) = FMaxOut);
end;
...
{$IFDEF DEBUG} SetLength(DebugArr, Length(Data)); {$ENDIF}
  ...
  if FAgcEnabled then for i := 0 to High(Result) do
    begin
